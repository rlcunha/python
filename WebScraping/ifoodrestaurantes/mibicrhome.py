...
class ChromeAuto:
    ...
    def digita_endereco(self):
        ...
        try:
            ...
            btn_localiza = self.chrome.find_element(
                By.CLASS_NAME, 'address-search-input__button')
    ...
    def seleciona_endereco(self):
        ...
        try:
            btn_endereco = self.chrome.find_element(
                By.XPATH, '/html/body/div[4]/div/div/div/div/div/div[2]/div/div[1]/div[3]/ul/li[1]/div/button')
            btn_endereco.click()
    ...
    def confirma_endereco(self):
        ...
        try:
            btn_confirma = self.chrome.find_element(
                By.XPATH, '/html/body/div[4]/div/div/div/div/div/div[3]/div[2]/button/span')
            btn_confirma.click()
    ...
    def salva_endereco(self):
        ...
        try:
            btn_confirma = self.chrome.find_element(
                By.XPATH, '/html/body/div[4]/div/div/div/div/div/div[3]/div[1]/div[2]/form/div[4]/button/span')
            btn_confirma.click()
    ...
    def gravar(self):
        try:
            ...
            fileNamejson = f'ifood-{t}-{self.cidade.replace(" ", "")}-{self.bairro.replace(" ", "")}.json'
            print('gravar')
            self.df.to_json(fileNamejson, orient='records')
            return fileNamejson

        except Exception as e:
            print('Erro gravar dataframe:', e)

    def gravar_dados_cloud(self, fileCloud):
        try:
            print('gravar_dados_cloud')
            CONNECT_STR = "DefaultEndpointsProtocol=https;AccountName=sagestaoloja;AccountKey=+SnI3A23dIH40v2fM5e3bfkMpQgVEhB4779NZQ/wIogJhMcga+qf3gwT3dUnxiAvX5diVF+XE2Vl+AStKcxJPA==;EndpointSuffix=core.windows.net"
            CONTAINER_NAME = "landing"
            input_file_path = fileCloud
            output_blob_name = fileCloud
    ...
    def ver_mais(self):
        print('ver_mais')
        try:
            btn_ver_mais = self.chrome.find_element(
                By.CLASS_NAME, 'cardstack-nextcontent')
            btn_ver_mais.click()
    ...
    def carrega_restaurante(self):
        ...
        try:
            conteudo = self.chrome.find_element(
                By.CLASS_NAME, 'merchant-list-v2__wrapper').get_attribute('outerHTML')
            ...
            self.df = pd.DataFrame(columns=['titulo', 'frete', 'entregaDe', 'entregaAte', 'data', 'dia', 'mes', 'ano',
                                   'cliente', 'pontos', 'tipo', 'distancia', 'cidade', 'bairro', 'entreDe', 'entreAte'])
                ...
                for d in foo_descendants:
                    if d.name == 'div':
                        ...
                        if d.get('class', '') == ['merchant-v2__footer']:
                            footer = d.text.replace("´", "").replace("`", "").replace(
                                "'", "").replace("á", "a").replace(",", ".").replace(r"\xa", " ")
                            self.df = self.df.append({'titulo': header,
                                                      ...
                                                      'dia': time.strftime(r"%d", time.localtime()),
                                                      'mes': time.strftime(r"%m", time.localtime()),
                                                      'ano': time.strftime(r"%Y", time.localtime()),
        ...
        try:
            ...
            if tipo == 'entregaDe':
                if 'Hoje' not in dados and 'Fechado' not in dados:
                    ret = pd.to_numeric(dados.split(sep=" ")[
                                        0].split(sep="-")[0])
            ...
            if tipo == 'entreDe':
                if 'Hoje' in dados:
                    ret = pd.to_numeric(dados.replace(
                        "h", "").split(sep=" ")[2].split(sep="-")[0])

            if tipo == 'entreAte':
                if 'Hoje' in dados:
                    ret = pd.to_numeric(dados.replace(
                        "h", "").split(sep=" ")[2].split(sep="-")[1])
